...
export const PhotosGallery: React.FC<{ initialSport?: string; onBackToHome: () => void }> = ({
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    setUploading(true);
    try {
      let finalStorageUrl = imageSrc;

      // If it's a data URL, upload to Firebase Cloud Storage bucket to get permanent download URL
      if (imageSrc.startsWith('data:')) {
        finalStorageUrl = await uploadBase64Image(imageSrc, 'photos');
      }
