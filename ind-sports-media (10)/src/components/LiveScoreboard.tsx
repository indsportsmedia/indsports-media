...
export default function LiveScoreboard({ onBack }: LiveScoreboardProps) {
  ...
  const handlePublishStory = async () => {
    ...
    try {
      ...
      // 1. STEP A: Securely upload media to Firebase Cloud Storage first
      if (storyMedia.file) {
        // Direct upload of the File/Blob using uploadBytesResumable
        finalStorageUrl = await uploadMediaFile(
          storyMedia.file,
          storyMedia.type === 'video' ? 'videos' : 'highlights',
          (pct) => setUploadProgress(Math.min(90, Math.max(15, pct)))
        );
      } else if (storyMedia.url.startsWith('data:')) {
        // Upload Base64 data string to Firebase Storage bucket using uploadString
        finalStorageUrl = await uploadBase64Image(
          storyMedia.url,
          'highlights',
          (pct) => setUploadProgress(Math.min(90, Math.max(15, pct)))
        );
      } else if (storyMedia.url.startsWith('http')) {
        // Already a hosted URL
        finalStorageUrl = storyMedia.url;
      }

      // Verification: Make sure we received a valid hosted HTTPS download link from Firebase Storage
      if (!finalStorageUrl || finalStorageUrl.startsWith('data:')) {
        throw new Error('Firebase Storage did not return a valid download URL. Check Storage bucket permissions.');
      }
      ...
      // Lightweight clip metadata (storing only the public storage URL)
      ...
      // Strip any legacy base64 entries from local clip history to keep Firestore doc <50KB
      const cleanExistingClips = (activeGame.socialClips || []).filter(c => !c.mediaUrl?.startsWith('data:') && !c.url?.startsWith('data:'));

      const updatedGame: GameState = {
        ...activeGame,
        socialClips: [newClip, ...cleanExistingClips]
      };

      // 2. STEP B: Write only the clean metadata & short download link to Firestore
      await broadcastGameUpdate(updatedGame);

      // 3. STEP C: Also sync the short Storage URL to the global Photos Gallery if it's a photo
      ...
      setTimeout(() => {
        ...
        alert('🎉 Posted! Your highlight is live across all scoreboards.');
      }, 300);

    } catch (err: any) {
      console.error('Error publishing story highlight:', err);
      const msg = err?.message || 'Check stadium cellular connection.';
      alert(`⚠️ Upload Error: ${msg}`);
    } finally {
